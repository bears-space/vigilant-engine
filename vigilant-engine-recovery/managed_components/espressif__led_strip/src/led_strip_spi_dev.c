...
static esp_err_t led_strip_spi_set_pixel(led_strip_t* strip, uint32_t index,
                                         ...
                                         uint32_t blue) {
    led_strip_spi_obj* spi_strip =
        __containerof(strip, led_strip_spi_obj, base);
    ...
    for (uint8_t i = 0; i < format.bytes_per_color; i++) {
        ...
        __led_strip_spi_bit(
            (red >> color_shift) & 0xFF,
            &pixel_buf[start + SPI_BYTES_PER_COLOR_BYTE *
                                   (format.r_pos * pos_bytes + i)]);
        __led_strip_spi_bit(
            (green >> color_shift) & 0xFF,
            &pixel_buf[start + SPI_BYTES_PER_COLOR_BYTE *
                                   (format.g_pos * pos_bytes + i)]);
        __led_strip_spi_bit(
            (blue >> color_shift) & 0xFF,
            &pixel_buf[start + SPI_BYTES_PER_COLOR_BYTE *
                                   (format.b_pos * pos_bytes + i)]);
        if (format.num_components > 3) {
            __led_strip_spi_bit(
                0, &pixel_buf[start + SPI_BYTES_PER_COLOR_BYTE *
                                          (format.w_pos * pos_bytes + i)]);
        }
    }
    return ESP_OK;
}
...
static esp_err_t led_strip_spi_set_pixel_rgbw(led_strip_t* strip,
    ...
    ESP_RETURN_ON_FALSE(index < spi_strip->strip_len, ESP_ERR_INVALID_ARG, TAG,
                        "index out of maximum number of LEDs");
    ESP_RETURN_ON_FALSE(format.num_components == 4, ESP_ERR_INVALID_ARG, TAG,
...
static esp_err_t led_strip_spi_refresh(led_strip_t* strip) {
    ...
    ESP_RETURN_ON_ERROR(spi_device_transmit(spi_strip->spi_device, &tx_conf),
                        TAG, "transmit pixels by SPI failed");
...
static esp_err_t led_strip_spi_clear(led_strip_t* strip) {
    ...
    for (int index = 0;
         index < spi_strip->strip_len * spi_strip->bytes_per_pixel; index++) {
...
esp_err_t led_strip_new_spi_device(const led_strip_config_t* led_config,
    ...
    ESP_GOTO_ON_FALSE(led_config && spi_config && ret_strip,
                      ESP_ERR_INVALID_ARG, err, TAG, "invalid argument");
    led_color_component_format_t component_fmt =
        led_config->color_component_format;
    ...
    if (component_fmt.format.num_components == 3) {
        mask = BIT(component_fmt.format.r_pos) |
               BIT(component_fmt.format.g_pos) |
               BIT(component_fmt.format.b_pos);
        // Check for invalid values
        ESP_RETURN_ON_FALSE(mask == 0x07, ESP_ERR_INVALID_ARG, TAG,
                            "invalid order argument");
    } else if (component_fmt.format.num_components == 4) {
        mask =
            BIT(component_fmt.format.r_pos) | BIT(component_fmt.format.g_pos) |
            BIT(component_fmt.format.b_pos) | BIT(component_fmt.format.w_pos);
        // Check for invalid values
        ESP_RETURN_ON_FALSE(mask == 0x0F, ESP_ERR_INVALID_ARG, TAG,
                            "invalid order argument");
    } else {
        ESP_RETURN_ON_FALSE(false, ESP_ERR_INVALID_ARG, TAG,
                            "invalid number of color components: %d",
                            component_fmt.format.num_components);
    }
    ...
    ESP_GOTO_ON_ERROR(
        spi_bus_initialize(
            spi_strip->spi_host, &spi_bus_cfg,
    ...
    ESP_GOTO_ON_FALSE((clock_resolution_khz <
                       LED_STRIP_SPI_DEFAULT_RESOLUTION / 1000 + 300) &&
                          (clock_resolution_khz >
                           LED_STRIP_SPI_DEFAULT_RESOLUTION / 1000 - 300),
                      ESP_ERR_NOT_SUPPORTED, err, TAG,
                      "unsupported clock resolution:%dKHz",
                      clock_resolution_khz);

    if (led_config->led_model != LED_MODEL_WS2812) {
        ESP_LOGW(TAG,
                 "Only support WS2812. The timing requirements for other "
                 "models may not be met");
    }
