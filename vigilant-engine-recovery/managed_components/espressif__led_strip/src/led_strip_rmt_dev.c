...
static esp_err_t led_strip_rmt_set_pixel(led_strip_t* strip, uint32_t index,
    ...
    led_strip_rmt_obj* rmt_strip =
        __containerof(strip, led_strip_rmt_obj, base);
    ...
    for (uint8_t i = 0; i < format.bytes_per_color; i++) {
        ...
        pixel_buf[start + format.r_pos * pos_bytes + i] =
            (red >> color_shift) & 0xFF;
        pixel_buf[start + format.g_pos * pos_bytes + i] =
            (green >> color_shift) & 0xFF;
        pixel_buf[start + format.b_pos * pos_bytes + i] =
...
static esp_err_t led_strip_rmt_set_pixel_rgbw(led_strip_t* strip,
    ...
    for (uint8_t i = 0; i < format.bytes_per_color; i++) {
        ...
        pixel_buf[start + format.b_pos * pos_bytes + i] =
            (blue >> color_shift) & 0xFF;
        pixel_buf[start + format.w_pos * pos_bytes + i] =
...
esp_err_t led_strip_new_rmt_device(const led_strip_config_t* led_config,
    ...
    ESP_GOTO_ON_FALSE(led_config && rmt_config && ret_strip,
    ...
    rmt_strip = calloc(
        1, sizeof(led_strip_rmt_obj) + led_config->max_leds * bytes_per_pixel);
    ...
    ESP_GOTO_ON_ERROR(
        rmt_new_tx_channel(&rmt_chan_config, &rmt_strip->rmt_chan), err, TAG,
        "create RMT TX channel failed");

    led_strip_encoder_config_t strip_encoder_conf = {
        .resolution = resolution, .led_model = led_config->led_model};
    ESP_GOTO_ON_ERROR(rmt_new_led_strip_encoder(&strip_encoder_conf,
