 ...
 * @note The format is used to specify the order of color components in each
 * pixel, also the number of color components.
 */
typedef union {
    struct format_layout {
        uint32_t r_pos
            : 2; /*!< Position of the red channel in the color order: 0~3 */
        uint32_t g_pos
            : 2; /*!< Position of the green channel in the color order: 0~3 */
        uint32_t b_pos
            : 2; /*!< Position of the blue channel in the color order: 0~3 */
        uint32_t w_pos
            : 2; /*!< Position of the white channel in the color order: 0~3 */
        uint32_t reserved : 19;       /*!< Reserved */
        uint32_t bytes_per_color : 2; /*!< Bytes per color component: 1 or 2. If
                                         set to 0, it will fallback to 1 */
        uint32_t num_components
            : 3; /*!< Number of color components per pixel: 3 or 4. If set to 0,
                    it will fallback to 3 */
...
typedef struct {
    ...
    uint32_t max_leds;  /*!< Maximum number of LEDs that can be controlled in a
                           single strip */
    led_model_t
        led_model; /*!< Specifies the LED strip model (e.g., WS2812, SK6812) */
    led_color_component_format_t
        color_component_format; /*!< Specifies the order of color components in
                                   each pixel. Use helper macros like
                                   `LED_STRIP_COLOR_COMPONENT_FMT_GRB` to set
                                   the format */
